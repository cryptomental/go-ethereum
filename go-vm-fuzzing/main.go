...
//export addAccount
func addAccount(address []byte, balance uint64, code []byte, storage map[common.Hash]common.Hash) {
	account := AccountData{
		address: address,
		balance: balance,
		code:    code,
		...
		*/
		if do_trace != 0 {
			...
			for _, t := range logs {
				...
				fmt.Printf("Storage:\n")
				for i, storageValue := range t.Storage {
					fmt.Sprintf("%d %x", i, storageValue)
				}
				fmt.Printf("\n")
				execution_num++
			}
